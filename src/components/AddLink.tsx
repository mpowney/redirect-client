...
const styles = require("styles/components/AddLink.less");
    ...
    render() {
        ...
        return (
            <div className={styles.addLink}>
                <TextField defaultValue={this.state.redirectTo} label={`Redirect to`} placeholder={`Enter the URL to link to`} className={styles.LinkField} onChange={(event: React.FormEvent, value: string) => this.updateState(event, `redirectTo`, value) } />
                <TextField value={this.state.shortName} label={`Short name`} prefix={`${this.state.shortPrefix}`} placeholder={`Leave blank to generate random short name`} className={styles.ShortNameField} onChange={(event: React.FormEvent, value: string) => this.updateState(event, `shortName`, value) } iconProps={{ iconName: 'Copy', onClick: this.copyClick }} />
